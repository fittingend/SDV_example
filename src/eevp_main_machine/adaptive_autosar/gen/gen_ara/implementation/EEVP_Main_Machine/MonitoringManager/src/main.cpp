...
/// EXECUTABLE NAME                   : MonitoringManager
...
monitoringmanager::aa::MonitoringManager* g_swcMonitoringManager{nullptr};
 
static void SignalHandler(std::int32_t signal)
{
    if (signal == SIGTERM || signal == SIGINT)
    {
        g_swcMonitoringManager->Terminate();
    }
}
...
int main(int argc, char *argv[], char* envp[])
{
    ...
    if (araInitialized)
    {
        ara::log::Logger& appLogger{ara::log::CreateLogger("MONM", "MonitoringManager's main function")};
        ...
        monitoringmanager::aa::MonitoringManager swcMonitoringManager;
        g_swcMonitoringManager = &swcMonitoringManager;
        
        // initialize software component
        proceed = swcMonitoringManager.Initialize();
        
        if (proceed)
        {
            // start software component
            swcMonitoringManager.Start();
