...
soa_smartfilm::aa::SOA_SmartFilm* g_swcSOA_SmartFilm{nullptr};
 
static void SignalHandler(std::int32_t signal)
{
    if (signal == SIGTERM || signal == SIGINT)
    {
        g_swcSOA_SmartFilm->Terminate();
    }
}
...
int main(int argc, char *argv[], char* envp[])
{
    ...
    if (araInitialized)
    {
        ara::log::Logger& appLogger{ara::log::CreateLogger("SSFM", "SOA_SmartFilm's main function")};
        ...
        soa_smartfilm::aa::SOA_SmartFilm swcSOA_SmartFilm;
        g_swcSOA_SmartFilm = &swcSOA_SmartFilm;
        
        // initialize software component
        proceed = swcSOA_SmartFilm.Initialize();
        
        if (proceed)
        {
            // start software component
            swcSOA_SmartFilm.Start();
