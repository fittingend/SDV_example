...
void BatteryMonitor_triggered_sys(B_batterymonitor_T batterymonitor_B, uint8_t *brightness)
{
    ...
    // if (R_BmsInfo) {
        ...
        // if (resultPtr->HasValue() && (resultPtr->Value() > 0)) {
            ...
            if (batterymonitor_B.RxTriggered.DataSerialNumber == 1U) {
                rtb_start = 0;
            }
            ...
            batterymonitor_B.FixPtRelationalOperator = (rtb_start >
                batterymonitor_DW.DelayInput1_DSTATE);

            // Outputs for Triggered SubSystem: '<S1>/Subsystem' incorporates:
            //   TriggerPort: '<S5>/Trigger'

            if (batterymonitor_B.FixPtRelationalOperator &&
                (batterymonitor_PrevZCX.Subsystem_Trig_ZCE != POS_ZCSIG)) {
                    *brightness = 0x9;
            }

            batterymonitor_PrevZCX.Subsystem_Trig_ZCE =
                batterymonitor_B.FixPtRelationalOperator;
            ...
            batterymonitor_B.FixPtRelationalOperator_p =
            ...
            if (batterymonitor_B.FixPtRelationalOperator_p &&
                (batterymonitor_PrevZCX.Subsystem1_Trig_ZCE != POS_ZCSIG)) {
                    *brightness = 0x0;
            }

            if(batterymonitor_B.RxTriggered.DataSerialNumber == 1)
                    *brightness = 0x0;

            batterymonitor_PrevZCX.Subsystem1_Trig_ZCE =
                batterymonitor_B.FixPtRelationalOperator_p;

            // End of Outputs for SubSystem: '<S1>/Subsystem1'

            // S-Function (aralog): '<S1>/ara::log1'
            // araLog->LogVerbose() << "'Start Trigger : '" <<
            //     batterymonitor_B.FixPtRelationalOperator;
            // ;

            // // S-Function (aralog): '<S1>/ara::log'
            // araLog->LogVerbose() << "'Finish Trigger : '" <<
            //     batterymonitor_B.FixPtRelationalOperator_p;
            // ;
            ...
            else if (batterymonitor_B.FixPtRelationalOperator_p) {
                rtb_ChargeStatus = 2U;
            }
            ...
            rtb_LogicalOperator = (batterymonitor_B.FixPtRelationalOperator ||
                batterymonitor_B.FixPtRelationalOperator_p);

            // Outputs for Triggered SubSystem: '<S1>/SendMessage' incorporates:
            //   TriggerPort: '<S4>/Trigger'

            if (rtb_LogicalOperator && (batterymonitor_PrevZCX.SendMessage_Trig_ZCE !=
                POS_ZCSIG)) {
            ...
            batterymonitor_PrevZCX.SendMessage_Trig_ZCE = rtb_LogicalOperator;
...
//
void batterymonitor_Convert_CurrMINdata()
{
}
...
//
void batterymonitor_Convert_CurrRCdata(const double rtu_u[20],
...
//
void batterymonitor_Convert_CurrYdata(const double rtu_u[100],
...
//
void batterymonitor_MATLABFunction(uint8_t rtu_y_prev, uint8_t
...
//
void batterymonitor_MATLABFunction10(uint8_t rtu_der, double
...
//
void batterymonitor_MATLABFunction4(uint32_t rtu_t, uint32_t
