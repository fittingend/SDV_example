...
preventrollback::aa::PreventRollback* g_swcPreventRollback{nullptr};
 
static void SignalHandler(std::int32_t signal)
{
    if (signal == SIGTERM || signal == SIGINT)
    {
        g_swcPreventRollback->Terminate();
    }
}
...
int main(int argc, char *argv[], char* envp[])
{
    ...
    if (araInitialized)
    {
        ...
        preventrollback::aa::PreventRollback swcPreventRollback;
        g_swcPreventRollback = &swcPreventRollback;
        
        // initialize software component
        proceed = swcPreventRollback.Initialize();
        
        if (proceed)
        {
            // start software component
            swcPreventRollback.Start();
